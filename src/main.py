...
class PandocFlowApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("PandocFlow - 现代化文档格式转换工具")
    ...
    def init_ui(self):
        ...
        app_title = QLabel("PandocFlow", self)
        ...
        self.combo_global_format.addItems(["docx", "md", "html", "pdf", "txt", "rst"])
    ...
    def log(self, message, level="info"):
        ...
        self.txt_log.appendHtml(f"<span style='color: {color};'>{prefix}{message}</span>")
        # Scroll to bottom
    ...
    def open_file_dialog(self):
        file_paths, _ = QFileDialog.getOpenFileNames(
            self, "选择要转换的文档", "", 
            "常用文档 (*.md *.markdown *.docx *.html *.htm *.txt *.rst);;所有文件 (*.*)"
        )
        if file_paths:
    ...
    def handle_files_dropped(self, file_paths):
        ...
        supported_exts = {'.md', '.markdown', '.docx', '.html', '.htm', '.txt', '.rst'}
        added_count = 0
    ...
    def add_file_to_table(self, file_path, size, ext):
        ...
        if ext_lower in ('.md', '.markdown'):
            targets = ["docx", "html", "pdf", "txt", "rst"]
        elif ext_lower == '.docx':
            targets = ["md", "html", "txt", "pdf"]
        elif ext_lower in ('.html', '.htm'):
            targets = ["docx", "md", "pdf", "txt"]
        elif ext_lower == '.rst':
            targets = ["docx", "md", "html", "pdf"]
        else:
            targets = ["docx", "md", "html", "pdf", "txt", "rst"]
